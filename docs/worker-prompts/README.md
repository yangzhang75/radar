...
7. **合规追溯（型式认证强制）**：每个实现标准要求的类/函数，KDoc 注释中标注对应标准条款（如 `// IEC 62388 §6.x` / `// IEC 61162-1 ED6 §8.x` / `// IMO A.823 §x`）。精确实现标准数值，缺具体值时 `TODO(待标准: <标准号 §条款>)` 占位并报告，**不许近似或臆造**。在 `docs/合规追溯矩阵.md` 追加你落实的条款行。原文 PDF 在 `桌面/雷达开发资料/标准资料/`。
